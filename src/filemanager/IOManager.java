...
public class IOManager {
    ...
    public static List<Path> getPathList() {
        ...
        try (Stream<Path> streamList = Files.list(IOManager.getAddressPath())) {
            streamList.forEach(e -> list.add(e.getFileName()));
        } catch (IOException e) {
    ...
    //check if value from address field is shorter than 3 chars (i.e. C: or C)
    ...
    public static boolean setPathOrOpen(String path) throws IOException {
        ...
        if (Files.exists(address)) {
            ...
            if (!Files.isDirectory(addressPath)) {
                ...
                moveBackPath();
                return true;
            } else return Files.isDirectory(addressPath);
        }
        return false;
    }
    ...
    //if it does not contain a dot then returns empty String
    ...
    public static String getFileNameOnly() {
        ...
        if (fileName.toString().contains(".")) {
            int dotIndex = fileName.toString().lastIndexOf(".");
            return fileName.toString().substring(0, dotIndex);
        }
        return addressPath.getFileName().toString();
    }

    //go to the upper address if possible
    //when the address field has less than 3 chars or is a root (C:\) then return false
    ...
    public static int editName(String newName) throws IOException {
        Path newNamePath = Paths.get(addressPath.getParent().toString(), newName + getExtension());
